...
public class SimpleDataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (utilisateurRepository.count() > 0) {
            System.out.println("SimpleDataSeeder: données déjà présentes, on saute le seeding.");
            return;
        }

        try {
            System.out.println("SimpleDataSeeder: démarrage du seeding...");
            ...
            System.out.println("SimpleDataSeeder: seeding terminé avec succès.");
        } catch (Exception e) {
            System.err.println("SimpleDataSeeder: erreur lors du seeding: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
